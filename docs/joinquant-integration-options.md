...
零基础用户先读[聚宽接入教程](beginner-route-b.md)：五步完成服务准备、账户查询和两种策略改法的选择。
...
以下是远程调用简写，不含回测分流。显式 `bt.xxx` 不自动识别聚宽运行环境；同一策略需要回测时，请采用[教程中的分流示例](beginner-route-b.md)，不要直接将下面的交易点用于回测。
